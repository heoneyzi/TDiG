...
def main():
    ...
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    modes = ["direction", "magnitude", "null", "full"]

    recs = run_mock(args, modes, rng) if args.mock else run_real(args, modes, rng)
    import pandas as pd
    from scipy.stats import wilcoxon
    df = pd.DataFrame(recs); df.to_csv(out / "patching_results.csv", index=False)
    ...
    for ell in args.layers:
        ...
        row = {"n": int(len(piv)),
               ...
               "median_null": float(piv["null"].median()),
               "direction_over_magnitude": float(piv["direction"].median() / max(piv["magnitude"].median(), 1e-12)),
               "direction_above_null": bool(piv["direction"].median() > piv["null"].median()),
               "magnitude_at_or_below_null": bool(piv["magnitude"].median() <= piv["null"].median())}
        try:
